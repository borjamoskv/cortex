...
Uses inline HTML/CSS/JS + Chart.js (CDN) — zero frontend build step.
"""
...
function renderDaemon(d) {
  ...
  if (d.status === 'no_data') {
    el.innerHTML = '<div class="daemon-row"><div class="daemon-indicator unknown"></div><span class="daemon-value">Not yet run</span></div>';
    return;
  }
...
async function doSearch(q) {
  ...
  try {
    ...
    if (!res.length) {
      el.innerHTML = '<div style="text-align:center; color:#666; padding:20px">No results found</div>';
      return;
    }
